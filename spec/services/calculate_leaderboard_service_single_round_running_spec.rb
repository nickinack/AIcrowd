...
describe CalculateLeaderboardService, api: true do
  ...
  describe 'order by primary desc, secondary desc' do
    ...
    it {
      expect(Leaderboard.first.previous_row_num)
          .to eq(2)
    }

    it {
      expect(Leaderboard.first.score)
          .to eq(36.00)
    }

    it {
      expect(Leaderboard.first.score_secondary)
          .to eq(0.003)
    }

    it {
      expect(Leaderboard.first.entries)
          .to eq(2)
    }

    it {
      expect(Leaderboard.first.post_challenge)
          .to eq(false)
    }

    it {
      expect(Leaderboard.second.participant_id)
    ...
    it {
      expect(Leaderboard.second.previous_row_num)
          .to eq(1)
    }

    it {
      expect(Leaderboard.second.score)
          .to eq(32.05)
    }

    it {
      expect(Leaderboard.second.score_secondary)
          .to eq(0.002)
    }

    it {
      expect(Leaderboard.second.entries)
          .to eq(4)
    }
